...
public final class CurrencyConverter {
	private Logger logger = Logger.getLogger(CurrencyConverter.class.getName());

	private static CurrencyConverter instance = null;

	private String ecbRatesURL = "http://127.0.0.1/hin-web/html/currency.xml";
	...
	private String referenceCurrency = "USD";
	 ...
	 */
	public static CurrencyConverter getInstance(String ecbRatesURL) {
		if (instance == null) {
			instance = new CurrencyConverter();
			instance.setEcbRatesURL(ecbRatesURL);
		}
		return instance;
	}
	 ...
	 * another. Example: convert(29.95, "USD", "EUR") - converts $29.95 US
	 * Dollars to Euro.
	 * 
	 * @param amount
	...
	*//**
	...
	private void update() throws IOException, ParseException {
		/*
		 * if (referenceDate == null) { initCacheFile(); if
		 * (!cacheFile.exists()) { refreshCacheFile(); } parse(); } if
		 * (cacheIsExpired()) { refreshCacheFile(); parse(); }
		 */
		parse();
	}
	...
	private void parse() throws ParseException {
		try {
			...
			try {
				URL ecbRates = new URL(ecbRatesURL);
				input = new InputStreamReader(ecbRates.openStream());
				...
				lastError = null;
				fxRates.clear();
				fxRates.put(referenceCurrency, 10000L);
				saxReader.setContentHandler(handler);
				saxReader.setErrorHandler(handler);
				saxReader.parse(new InputSource(input));
				input.close();
			} catch (Exception e) {
				logger.error("Currency Conversion Parse Error" , e);
			}
			if (lastError != null) {
