...
( function(a) {
	...
	a
			.extend(
					a.mobile.datebox.prototype._build,
					{
						...
						datebox : function() {
							var s = this, f = this.options, k, q, l, g = -2, n = "ui-datebox-", p = a("<div>"), c = a("<fieldset>"), h = p
									.clone(), j = c.clone(), e = a(
									"<input type='" + s.inputType + "' />")
									.addClass(
											"ui-input-text ui-corner-all ui-shadow-inset ui-body-"
													+ f.themeInput), r = e
									.clone().attr("type", "text"), m = a("<div>"), b = {
								theme : f.themeButton,
								icon : "plus",
								iconpos : "bottom",
								corners : true,
							...
							if (typeof s.d.intHTML !== "boolean") {
								s.d.intHTML.empty().remove()
							}
							s.d.headerText = ((s._grabLabel() !== false) ? s
									._grabLabel() : ((f.mode === "datebox") ? s
									.__("titleDateDialogLabel") : s
									.__("titleTimeDialogLabel")));
							s.d.intHTML = a("<span>");
							if (s.inputType !== "number") {
								e.attr("pattern", "[0-9]*")
							}
							s.fldOrder = ((f.mode === "datebox") ? s
									.__("dateFieldOrder") : s
									.__("timeFieldOrder"));
							s._check();
							s._minStepFix();
							s
									._dbox_vhour(typeof s._dbox_delta !== "undefined" ? s._dbox_delta
											: 1);
							if (f.mode === "datebox") {
								a(
										'<div class="'
							...
							for (k = 0; k <= s.fldOrder.length; k++) {
								...
								case "d":
								case "h":
									a("<div>").append(s._makeEl(e, {
										attr : {
											field : s.fldOrder[k],
											amount : 1
										}
									})).addClass("ui-block-" + l).appendTo(h);
									s._makeEl(m, {
										attr : {
											field : s.fldOrder[k],
											amount : 1
									...
									s._makeEl(m, {
										attr : {
											...
											.buttonMarkup(d).appendTo(j);
									g++;
									break;
								case "a":
									if (s.__("timeFormat") === 12) {
										a("<div>").append(s._makeEl(r, {
											attr : {
												field : s.fldOrder[k],
												amount : 1
											}
										})).addClass("ui-block-" + l).appendTo(
												h);
										s._makeEl(m, {
											attr : {
												field : s.fldOrder[k],
											...
											function() {
												...
												case "M":
													a(this)
															.val(
																	s
																			.__("monthsOfYearShort")[s.theDate
																			.getMonth()]);
													break;
												case "a":
													a(this)
							...
							if (s.dateOK !== true) {
								h.find("input")
										.addClass(n + "griddate-disable")
							} else {
								h.find("." + n + "griddate-disable")
										.removeClass(n + "griddate-disable")
							}
							if (f.useSetButton || f.useClearButton) {
								...
								if (f.useClearButton) {
									a(
											...
											.buttonMarkup( {
												...
												corners : true,
												shadow : true
											}).on(f.clickEventAlt, function(i) {
												i.preventDefault();
												s.d.input.val("");
												s.d.input.trigger("datebox", {
													method : "clear"
												});
												s.d.input.trigger("datebox", {
													method : "close"
								...
								if (f.useCollapsedBut) {
									q.addClass("ui-datebox-collapse")
								}
								q.appendTo(s.d.intHTML)
							}
							if (f.repButton === false) {
								c.on(f.clickEvent, "div", function(i) {
									i.preventDefault();
									s._dbox_delta = 1;
									s._offset(a(this).jqmData("field"), a(this)
											.jqmData("amount"))
								});
								j.on(f.clickEvent, "div", function(i) {
									i.preventDefault();
									s._dbox_delta = -1;
									s._offset(a(this).jqmData("field"), a(this)
											.jqmData("amount")
							...
							h.on("change", "input", function() {
								s._dbox_enter(a(this))
							});
							if (s.wheelExists) {
								h.on("mousewheel", "input", function(i, o) {
									i.preventDefault();
									s._dbox_delta = o < 0 ? -1 : 1;
									s
							...
							if (f.repButton === true) {
								c.on(s.drag.eStart, "div", function(i) {
									l = [ a(this).jqmData("field"),
									...
									if (!s.runButton) {
										s.drag.target = l;
										s.runButton = setTimeout( function() {
											s._dbox_run()
										}, 500)
									}
								});
								j.on(s.drag.eStart, "div", function(i) {
									l = [ a(this).jqmData("field"),
	...
	a.extend(a.mobile.datebox.prototype._drag, {
		timebox : function() {
