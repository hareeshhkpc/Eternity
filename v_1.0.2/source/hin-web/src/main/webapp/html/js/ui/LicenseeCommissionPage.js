...
if (!HIN)
	HIN = {};
HIN.LicenseeCommissionPage = function(appController, pageHolder) {
	licenseeCommissionPage = this;
	this.appController = appController;
	this.processDefinition = null;
	this.messageTypes = [];
	this.selectedStep = null;
	this.pageHolder = "licenseeCommissionPage";// pageHolder;
...
HIN.LicenseeCommissionPage.prototype.init = function(callback, page) {
	this.page = page;
	if (callback) {
		callback(page);
	}
...
HIN.LicenseeCommissionPage.prototype.pageBeforeLoad = function(messageType,
		uiGenerator) {

	// alert("LicenseeCommissionPage pageBeforeLoad : " + uiGenerator);
	...
	uiGenerator.tableHeaders = licenseeCommissionPage.getProductTableHeader();
	// uiGenerator.taskHandler = licenseeCommissionPage.taskHandler;

		
 /*
		 * else if (messageType.typeName == "NewProducts") {
		 * uiGenerator.tableHeaders =
		 * licenseeCommissionPage.getProductTableHeader(); } else {
		 * uiGenerator.tableHeaders =
		 * licenseeCommissionPage.getServiceCostTableHeader(); }
		 */

};

HIN.LicenseeCommissionPage.prototype.addInitialHandler = function(uiGenerator) {
	uiGenerator.formRender = true;
	// alert("addInitialHandler");
};

HIN.LicenseeCommissionPage.prototype.addCompleteHandler = function(addNew,
		messageType, message, uiGenerator) {
	/*
	 * if (addNew) { if (messageType.typeName == "NewServices") {
	 * licenseeCommissionPage.page.getMessageScript(message, null, function(
...
/*
 * HIN.LicenseeCommissionPage.prototype.taskHandler = function(message, taskVO,
 * instance) { // alert("taskHandler"); };
 */



HIN.LicenseeCommissionPage.prototype.getProductTableHeader = function() {
...
HIN.LicenseeCommissionPage.prototype.taskHandler = function(message, taskVO,
	...
	var messageObjects = [];
	var messageTypes = licenseeCommissionPage.selectedStep.getMessageTypes();
	for ( var messageTypeIndex = 0; messageTypeIndex < messageTypes.length; messageTypeIndex++) {
		...
		for ( var messageIndex = 0; messageIndex < messages.length; messageIndex++) {
			...
			if (message.status == AppConstants.Status.ACTIVE) {
				for ( var index = 0; index < licenseeCommissionPage.invoiceMessageMap
						.length(); index++) {
	...
	// licenseeCommissionPage.saveProcess();
	if (invoiceMessage) {// TODO logic need to be change
		messageObjects.push(invoiceMessage);
	}
	if (messageObjects.length > 0) {
		var parameters = [ messageObjects ];
		licenseeCommissionPage.appController.getComponent("DataLayer")
				.createOrUpdateTasks(parameters);
	}

};

HIN.LicenseeCommissionPage.prototype.singleHandler = function(typeName, instance) {
	// alert("singleHandler");
	var page = appController.getComponent("RenderingEngine").getChildComponent(
			"Form").getPage();
	var messageObjects = [];

	if (licenseeCommissionPage.invoiceCreated == true) {

		for ( var index = 0; index < licenseeCommissionPage.invoiceMessageMap
				.length(); index++) {
			var map = licenseeCommissionPage.invoiceMessageMap.getItemAt(index);
			var indexNum = parseInt(index + 1);

			if (map.value === true) {

				var messageId = map.key;
				var message = licenseeCommissionPage.appController.getComponent(
						"DataLayer").getMessageObject(messageId);
				messageObjects.push(message);
			}
		}
		if (licenseeCommissionPage.invoiceMessage) {// TODO logic need to be
													// change
			messageObjects.push(licenseeCommissionPage.invoiceMessage);
		}
		licenseeCommissionPage.invoiceCreated = false;
	} else {
...
HIN.LicenseeCommissionPage.prototype.refreshHandler = function(instance) {
...
HIN.LicenseeCommissionPage.prototype.addToInvoice = function() {

	// alert("Service: " + licenseeCommissionPage.serviceCheckFlag + " Product: "
	// +
	// licenseeCommissionPage.productCheckFlag);
	if (licenseeCommissionPage.productCheckFlag === true
			|| licenseeCommissionPage.serviceCheckFlag === true) {

		licenseeCommissionPage.page.createMessage("FIAB_MT020000HT02", null,
				null, null, licenseeCommissionPage.messageCreationComplete, null);
	}
}

HIN.LicenseeCommissionPage.prototype.messageCreationComplete = function(
		newMessage, conceptLookup, message) {
	licenseeCommissionPage.page.getMessageScript(newMessage, null,
			licenseeCommissionPage.fillNewInvoice);
};

HIN.LicenseeCommissionPage.prototype.fillNewInvoice = function(messageTypeScript,
		object, newMessage) {
	...
	for ( var index = 0; index < licenseeCommissionPage.invoiceMessageMap
		...
		if (map.value === true) {
			...
			var messageId = map.key;
			var msg = licenseeCommissionPage.appController.getComponent(
	...
	licenseeCommissionPage.invoiceMessage = newMessage;
	licenseeCommissionPage.invoiceCreated = true;
	licenseeCommissionPage.openInvoice(newMessage.msg);

};

HIN.LicenseeCommissionPage.prototype.openInvoice = function(msg) {
	...
	invoiceForm += '<div>';
	invoiceForm += '<div id="hospitalName">Eternity Medical Institute Inc.Dubai<br>';
