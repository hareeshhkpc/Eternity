...
(function($) {
		...
		if (data) {
			$.each(data, function(index, value) {
				...
				if ($.isPlainObject(value)) {
					str.push('<li data-icon=' + settings.icon + '><a href="' + settings.link + encodeURIComponent(value.value) + '" data-value="'+value.value+'" data-transition="' + settings.transition + '">' + value.label + '</a></li>');
				} else {
	...
	handleInput = function(e) {
		...
		if (settings) {
			...
			if ((!e.keyCode==32 || text.length < settings.minLength) && e.which != 1) {
				var htmlContent = $(settings.target).html();
				if(htmlContent){
				...
				if ($.isArray(settings.source)) {
					...
					if (e.keyCode===32/* ||( text.length == 1 && !text[0]=="")*/){
						data = settings.source.sort(function(element) {
							var element_text, re = new RegExp('^' + text, 'i');
							if ($.isPlainObject(element)) {
								element_text = element.label;
							} else {
							...
							if($(textBox).val() < settings.minLength){
						          $(textBox).val("");								
							}
							return element_text;
						});
					...
					if (e.which == 1){
						var htmlContent = $(settings.target).html();
						if(htmlContent){
							clearTarget(text, $(settings.target));
						}else{
							data = settings.source.sort(function(element) {
								var element_text, re = new RegExp('^' + text, 'i');
								if ($.isPlainObject(element)) {
	...
	methods = {
		init: function(options) {
			...
			this.unbind("click.autocomplete").bind("click.autocomplete", handleInput);
