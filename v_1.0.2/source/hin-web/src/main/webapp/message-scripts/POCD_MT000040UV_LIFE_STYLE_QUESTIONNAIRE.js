...
function POCD_MT000040UV_LIFE_STYLE_QUESTIONNAIRE(appController, messageAndUIBinder, logger) {
	...
	function fillData(key, values) {
		...
		if(key === 'updateValues'){
			...
			$.each(values, function(index, updatedValue){
				
				if(updatedValue[0] == 'historyYes' || updatedValue[0] == 'historyNo' ){
					if(updatedValue[0] == 'historyYes')
						fields = "component,structuredBody,component,section,entry,observation[21]";
					else if(updatedValue[0] == 'historyNo')
						fields = "component,structuredBody,component,section,entry,observation[22]";
					type = "ED";
					tagName = "value";
					pathFields = fields.split(',');
					instanceObject = [updatedValue[1]];	
					thisObject.messageAndUIBinder.writeValueToMessage(tagName, pathFields, type, instanceObject);
				}else{
					fields = updatedValue[1];
					type = "ED";
					tagName = "value";
					pathFields = fields.split(',');
					if(updatedValue[0] == 'yes')
						instanceObject = ['1'];
					else
						instanceObject = ['0'];
					thisObject.messageAndUIBinder.writeValueToMessage(tagName, pathFields, type, instanceObject);
