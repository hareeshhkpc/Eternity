...
function ROLE_EMPLOYEE(appController, messageAndUIBinder, logger) {
	...
	function initialize() {
		...
		try {
			...
			var staff = appController.getComponent("Context").getStaff();
			appController.getComponent("Context").setSubscriberId(staff);
			
			/* STAFF_NAME */
			if(staffVo){
				fields = "employeePerson";
				type = "PN";
				tagName = "name";
			...
			instanceObject = [ 'SUBSCRIBER_ID', staff, organizationName ];
			...
			instanceObject = [ 'ROLE_NAME', 'EMPLOYEE', organizationName ];
			...
			alert("Role Employee error : " + error);
			if (thisObject.logger) {
