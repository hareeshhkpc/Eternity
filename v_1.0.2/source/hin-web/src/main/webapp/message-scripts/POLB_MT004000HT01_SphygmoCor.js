...
function POLB_MT004000HT01_SphygmoCor(appController, messageAndUIBinder, logger) {
	...
	function fillData(key, values) {
		...
		if(key === 'interpretationCode'){
			$.each(values,function(index,range){
				fields = "component2,observationEvent["+(index+1)+"],referenceRange,interpretationRange[1]";
				type = "CE";
				...
				fields = "component2,observationEvent["+(index+1)+"],referenceRange,interpretationRange[2]";
				type = "CE";
				tagName = "interpretationCode";
				pathFields = fields.split(',');
				instanceObject = [range[1],null];
				messageAndUIBinder.writeValueToMessage(tagName, pathFields, type, instanceObject);
			});
		}
	}
