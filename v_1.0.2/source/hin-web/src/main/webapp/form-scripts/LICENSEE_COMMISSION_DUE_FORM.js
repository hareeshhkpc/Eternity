function LICENSEE_COMMISSION_DUE_FORM(message, appController, uiGenerator) {
	...
	function onServiceComplete(deleteMode) {
		...
		if (!licenseePaymentPage.serviceTotalRender) {
			// appending total text box after new services and new products
			// alert(licenseePaymentPage.serviceTotalRender);
		...
		function summerFunction() {
			...
			serviceDiv.find('input[tagname=amt]').each(
					function() {
						...
						// if (chkValue == true) {
						var map = licenseePaymentPage.invoiceMessageMap.get(messageId);
						if (!map) {
							licenseePaymentPage.invoiceMessageMap.put(messageId,
									chkValue);
						} else {
							map.value = chkValue;
						}
						 ...
						 * } else {
						 * 
						 * var map = licenseePaymentPage.invoiceMessageMap
						 * .get(messageId); if (map) {
						 * licenseePaymentPage.invoiceMessageMap .removeItem(messageId); } }
						 */
						/*
			...
			// alert(sum);
			$("#total1").val(sum);
			licenseePaymentPage.serviceCheckFlag = pFlag;
		...
		if (!licenseePaymentPage.serviceTotalRender) {
			...
			$('#imgInvoice').unbind('click', licenseePaymentPage.addToInvoice);
			$('#imgInvoice').bind('click', licenseePaymentPage.addToInvoice);
		}

		// alert("CHeckFlag: " + serviceCheckFlag)

		licenseePaymentPage.serviceTotalRender = true;
	}
	;
