function FEE_FORM(message, appController, uiGenerator) {
	...
	function initialize() {
		...
		try {
			...
			if (messageAndUIBinder) {
				messageAndUIBinder.addEditorListener("serviceListener",
						serviceListenerHandler);
				//var value = $("#inner-uiform-" + message.id).find("#PNDisplayValue").attr('value');
				//alert("value:"+value);
				//$("#inner-uiform-" + message.id).find("#pPhysician").attr("value",value);
			}
	...
	function onServiceComplete(deleteMode) {
		...
		accountPage.serviceTotalRender = true;
	}
	;
