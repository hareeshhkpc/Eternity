function SERVICE_FORM(message, appController, uiGenerator) {
	...
	function conceptHandler(conceptLookup) {
		if (conceptLookup) {
			...
			if (uiGenerator) {
				...
				feeValue = (parseFloat(feeValue).toFixed(2)).toString();
	...
	function onLoad(callback) {
		...
		try {
			 ...
			 * .find("#PNDisplayValue").attr('value'); $("#inner-uiform-" +
			 * message.id).find("#pPhysician").attr("value", value);
			 */
