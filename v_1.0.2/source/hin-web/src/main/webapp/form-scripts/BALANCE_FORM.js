function BALANCE_FORM(message, appController, uiGenerator) {
	...
	function onLoad(callback) {
		...
		try {
			...
			/*
			 * $("#inner-uiform-" +
			 * message.id).find('fieldset[amountParent=payment]').find('input[tagName=amt]')
			 * .unbind('keyup', addTotal); $("#inner-uiform-" +
			 * message.id).find('fieldset[amountParent=payment]').find('input[tagName=amt]')
			 * .bind('keyup', addTotal);
			 */
	...
	function addTotal() {
		...
		/*
		 * var licenseeVO =
		 * appController.getComponent("Context").getLicenseeVO(); if
		 * (licenseeVO) { var data = parseFloat($(this).val())
		 * parseFloat(licenseeVO.exchangeRate); //
		 * appController.getComponent("DataLayer").currencyConvert($(this).val(), //
		 * "USD", licenseeVO.currencyCode, licenseeVO.exchangeRate, //
		 * function(data) { $("#inner-uiform-" + message.id).find(
		 * 'fieldset[amountParent=payment]').find('#UsdAmt').text(
