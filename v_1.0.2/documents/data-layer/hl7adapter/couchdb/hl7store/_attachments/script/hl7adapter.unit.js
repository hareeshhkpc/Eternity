...
function testStatic(){
   ...
   function doTest(raw){
       ok(raw.indexOf("{") == 0)
       HL.getStaticContent("json_test",doTestJson,log,false, HL.RETURN_OBJECT)

   }
   function doTestJson(json){
       ok(json.id == "test")
       HL.getStaticContent("xml_test",doTestXml,log,false, HL.RETURN_DOM )
   }

   function doTestXml(xml){
       ok(xml.firstChild.nodeName=="message")
       start()
   }
    ...
    function run(){
        ...
        HL.clearConfigStore(function(){
            HL.clearMessageStore(function(){
                HL.getStaticContent("json_test",doTest,log,false, HL.RETURN_RAW)
            })   
        })
    }
    this.run = run
...
function testDynamic(){
   ...
   function doTest(json){
       ok(json.id == "test")
       json.name = "testname" 
       HL.putDynamicContent("json_test", json, doTestPut, log)

   }
   function doTestPut(json){
       ok(json.json_test != null)
       HL.getDynamicContent("json_test",doTestChanged,log,true, HL.RETURN_OBJECT)
   }

   function doTestChanged(json){
       ok(json.name == "testname")
        HL.syncMessage(function(data) { onSyncComplete(data)}, onSyncFail)
   }
    ...
    function onSyncComplete(data){
       ok(data.get >= 2 && data.put==1) 
       HL.putDynamicContent("testxml", "<test><id/></test>", doPutXML, log);
 

    }

    function doPutXML(changes){
       ok(changes.testxml != null)
       HL.getDynamicContent("testxml",doGetXML,log,true, HL.RETURN_DOM)         

    }
    function doGetXML(xml){
       ok(xml.firstChild.nodeName == "test")
       var id2 = xml.createElement("id2")
       xml.firstChild.appendChild(id2)
       HL.putDynamicContent("testxml", xml, doPutXML2, log);
    }

    function doPutXML2(changes){
       ok(changes.testxml != null)
       HL.getDynamicContent("testxml",doGetXML2,log,true, HL.RETURN_DOM)         

    }

    function doGetXML2(xml){
       ok(xml.firstChild.childElementCount==2)
       start()
    ...
    function run(){
        ...
        HL.clearConfigStore(function(){
            HL.clearMessageStore(function(){
                HL.getDynamicContent("json_test",doTest,log,false, HL.RETURN_OBJECT)
            })   
        })
    }
    this.run = run
